...
from exceptions import InvalidParameterError, DataProcessingError, ModelEvaluationError, AnomalyDetectionError
...
def run_anomaly_detection(n_points: int = 10000, window_size: int = 5, q: float = 1e-4) -> Tuple[UnivariateDS, List[float]]:
    ...
    if n_points <= 0 or window_size <= 0 or q <= 0 or q >= 1:
        raise InvalidParameterError("Invalid input parameters")
    ...
    try:
        ...
        raise DataProcessingError(f"Error during data processing: {str(e)}")
...
def evaluate_model(ds: UnivariateDS, scores: List[float]) -> Tuple[float, float, float]:
    ...
    Raises:
        ModelEvaluationError: If there's an error during model evaluation.
    """
    try:
        ...
        raise ModelEvaluationError(f"Error during model evaluation: {str(e)}")
...
def plot_results(ds: UnivariateDS, scores: List[float]) -> None:
    ...
    Raises:
        AnomalyDetectionError: If there's an error during plotting.
    """
    try:
        threshold = np.percentile(scores, 95)
        detected_anomalies = np.array(scores) > threshold
        ...
        raise AnomalyDetectionError(f"Error during result plotting: {str(e)}")
